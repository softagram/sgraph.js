import { CypherGraph, NodeData, EdgeData } from './graph';
import {
  CypherQuery,
...
export class CypherExecutor {
  ...
  execute(query: CypherQuery): CypherResult {
    ...
    if (query.whereClause) {
      bindings = bindings.filter((binding) =>
        this.evaluateWhere(query.whereClause!, binding)
      );
    }

    // 3. RETURN projection (with aggregation detection)
    let { columns, rows } = this.projectReturn(
      query.returnClause.items,
      bindings
    );
  ...
  private matchPattern(pattern: PatternElement[]): Binding[] {
    if (pattern.length === 0) return [];

    const firstNode = pattern[0] as NodePattern;
    ...
    for (let i = 1; i < pattern.length; i += 2) {
      const relPattern = pattern[i] as RelationshipPattern;
      const nextNodePattern = pattern[i + 1] as NodePattern;
      bindings = this.expandBindings(bindings, relPattern, nextNodePattern, pattern[i - 1] as NodePattern);
    }

    return bindings;
  }

  private nodeMatchesPattern(node: NodeData, pattern: NodePattern): boolean {
    // Check labels: every label in pattern must be in node's labels
    for (const label of pattern.labels) {
      if (!node.labels.has(label)) return false;
    }
  ...
  private expandBindings(
    ...
    for (const binding of bindings) {
      ...
      if (!prevVarName) continue;
      const prevEntry = binding.get(prevVarName);
      if (!prevEntry || prevEntry.kind !== 'node') continue;
      ...
      for (const edgeId of edgeIds) {
        ...
        if (relPattern.types.length > 0) {
          if (!relPattern.types.includes(edge.type)) continue;
        }

        // Get the other end of the edge
        const targetNodeId = this.getTargetNode(
          edgeId,
          prevNodeId,
        ...
        if (!this.nodeMatchesPattern(targetNode, nextNodePattern)) continue;

        // Create expanded binding
        const newBinding: Binding = new Map(binding);
  ...
  private getDirectedEdges(
    ...
    if (direction === 'out') return this.graph.getOutEdges(nodeId);
    if (direction === 'in') return this.graph.getInEdges(nodeId);
    // both: combine out + in
    return [
  ...
  private getTargetNode(
    ...
    if (direction === 'out') return this.graph.getEdgeDst(edgeId);
    if (direction === 'in') return this.graph.getEdgeSrc(edgeId);
  ...
  private evaluateWhere(expr: WhereExpr, binding: Binding): boolean {
    const result = this.evaluateExpr(expr, binding);
    return !!result;
  }

  private evaluateExpr(expr: WhereExpr, binding: Binding): any {
    switch (expr.kind) {
      case 'literal':
        return (expr as Literal).value;

      case 'variable': {
        const varRef = expr as VariableRef;
      ...
      case 'logical': {
        const logical = expr as LogicalExpr;
        if (logical.op === 'AND') {
          return (
            this.evaluateWhere(logical.left, binding) &&
  ...
  private resolveBindingEntry(
    ...
    if (expr.kind === 'variable') {
      return binding.get((expr as VariableRef).name);
    }
    return undefined;
  }
  ...
  private compareValues(left: any, right: any, operator: string): boolean {
    ...
    if (Array.isArray(left)) {
      ...
      return left.some((item: any) =>
        this.compareScalar(item, right, operator)
      );
    }
  ...
  private compareScalar(left: any, right: any, operator: string): boolean {
    switch (operator) {
      case '=':
        return left === right;
      case '<>':
  ...
  private projectReturn(
    ...
    const hasAggregation = items.some((item) =>
      this.isAggregation(item.expression)
    );
    ...
    for (const item of items) {
      if (
        ...
        (item.expression as VariableRef).name === '*'
      ) {
        // Expand wildcard: use first binding's keys
        if (bindings.length > 0) {
          for (const varName of bindings[0].keys()) {
            if (!columns.includes(varName)) columns.push(varName);
          }
        }
      } else {
      ...
      for (const item of items) {
        if (
          ...
          for (const [varName, entry] of binding) {
            row[varName] = this.resolveReturnValue(
  ...
  private isAggregation(expr: WhereExpr): boolean {
  ...
  private projectWithAggregation(
    ...
    const columns: string[] = items.map((item) => this.getColumnName(item));

    // Separate group-by keys from aggregations
    const groupByItems: ReturnItem[] = [];
    const aggItems: { item: ReturnItem; index: number }[] = [];
    ...
    if (groupByItems.length === 0) {
      ...
      for (const binding of bindings) {
        ...
        for (const gbItem of groupByItems) {
          keyParts.push(
            this.resolveReturnValue(gbItem.expression, binding)
          );
        }
    ...
    for (const [, groupBindings] of groups) {
      ...
      for (const item of items) {
        const colName = this.getColumnName(item);
        if (this.isAggregation(item.expression)) {
          const fc = item.expression as FunctionCall;
          if (fc.name === 'COUNT') {
            row[colName] = groupBindings.length;
          } else if (fc.name === 'COLLECT') {
            row[colName] = groupBindings.map((b) =>
              this.resolveReturnValue(fc.argument, b)
            );
          }
        } else {
          // Non-aggregated: take from first binding in group
          row[colName] = this.resolveReturnValue(
            item.expression,
            groupBindings[0]
          );
        }
  ...
  private getColumnName(item: ReturnItem): string {
    if (item.alias) return item.alias;
    return this.exprColumnName(item.expression);
  }

  private exprColumnName(expr: WhereExpr): string {
    switch (expr.kind) {
      case 'propertyAccess': {
        const pa = expr as PropertyAccess;
      ...
      case 'variable': {
        return (expr as VariableRef).name;
      }
      case 'functionCall': {
        const fc = expr as FunctionCall;
        if (fc.name === 'TYPE' && fc.argument.kind === 'variable') {
          return `type(${(fc.argument as VariableRef).name})`;
        }
        if (fc.name === 'COUNT') {
  ...
  private resolveReturnValue(expr: WhereExpr, binding: Binding): any {
    switch (expr.kind) {
      case 'propertyAccess': {
        const pa = expr as PropertyAccess;
        const entry = binding.get(pa.variable);
